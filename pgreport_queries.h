...
#define SETTINGS_NOTCONFIGFILE_NOTDEFAULTVALUE_SQL "SELECT source, name, setting, unit FROM pg_settings WHERE source NOT IN ('configuration file', 'default') OR (setting != boot_val) ORDER BY source, name"

#define CLUSTER_HITRATIO_TITLE "Hit ratio"
#define CLUSTER_HITRATIO_SQL "SELECT 'index hit rate' AS name, round(100.*sum(idx_blks_hit) / nullif(sum(idx_blks_hit + idx_blks_read),0),2) AS ratio FROM pg_statio_user_indexes UNION ALL SELECT 'table hit rate' AS name, round(100.*sum(heap_blks_hit) / nullif(sum(heap_blks_hit) + sum(heap_blks_read),0),2) AS ratio FROM pg_statio_user_tables"
...
#define PGSETTINGS_SQL "select category, name, setting, unit, source, boot_val, reset_val from pg_settings order by category"
...
#define PERCENTUSEDINDEXES_SQL "SELECT relname, CASE idx_scan WHEN 0 THEN 'Insufficient data' ELSE round(100. * idx_scan / (seq_scan + idx_scan),1)::text END percent_of_times_index_used, n_live_tup rows_in_table FROM pg_stat_user_tables ORDER BY n_live_tup DESC"
...
#define CORRUPTEDINDEXES_TITLE "Corrupted indexes"
#define CORRUPTEDINDEXES_SQL "SELECT * FROM get_corruptedindexes()"
...
#define VACUUMURGENCY_TITLE "VACUUM urgency"
#define VACUUMURGENCY_SQL "SELECT schemaname, relname, total_rows, n_dead_tup, coalesce(n_dead_tup / nullif(least(max_threshold, total_rows * scale_factor + threshold), 0), 100.0) AS vacuum_urgency, last_autovacuum FROM (SELECT st.schemaname, st.relname, t.reltuples AS total_rows, st.n_dead_tup, coalesce( min(split_part(ro.o, '=', 2)) FILTER (WHERE split_part(ro.o, '=', 1) = 'autovacuum_vacuum_scale_factor'), current_setting('autovacuum_vacuum_scale_factor'))::float8 AS scale_factor, coalesce( min(split_part(ro.o, '=', 2)) FILTER (WHERE split_part(ro.o, '=', 1) = 'autovacuum_vacuum_threshold'), current_setting('autovacuum_vacuum_threshold'))::float8 AS threshold, coalesce( min(split_part(ro.o, '=', 2)) FILTER (WHERE split_part(ro.o, '=', 1) = 'autovacuum_vacuum_max_threshold'), current_setting('autovacuum_vacuum_max_threshold', TRUE), 'Infinity')::float8 AS max_threshold, st.last_autovacuum FROM pg_stat_all_tables AS st JOIN pg_class AS t ON st.relid = t.oid LEFT JOIN LATERAL unnest(t.reloptions) AS ro(o) ON ro.o ~~ ANY ('{autovacuum_vacuum_scale_factor=%,autovacuum_vacuum_threshold=%,autovacuum_vacuum_max_threshold=%}') GROUP BY t.oid, st.schemaname, st.relname, t.reltuples, st.n_dead_tup, st.last_autovacuum) AS subq ORDER BY vacuum_urgency DESC"

#define ANALYZEURGENCY_TITLE "ANALYZE urgency"
#define ANALYZEURGENCY_SQL "SELECT schemaname, relname, total_rows, n_mod_since_analyze, n_mod_since_analyze / (total_rows * scale_factor + threshold) AS analyze_urgency, last_autoanalyze FROM (SELECT st.schemaname, st.relname, t.reltuples AS total_rows, st.n_mod_since_analyze, coalesce( min(split_part(ro.o, '=', 2)) FILTER (WHERE split_part(ro.o, '=', 1) = 'autovacuum_analyze_scale_factor'), current_setting('autovacuum_analyze_scale_factor'))::float8 AS scale_factor, coalesce( min(split_part(ro.o, '=', 2)) FILTER (WHERE split_part(ro.o, '=', 1) = 'autovacuum_analyze_threshold'), current_setting('autovacuum_analyze_threshold'))::float8 AS threshold, st.last_autoanalyze FROM pg_stat_all_tables AS st JOIN pg_class AS t ON st.relid = t.oid AND t.relkind = 'r' AND t.oid <> 2619 LEFT JOIN LATERAL unnest(t.reloptions) AS ro(o) ON ro.o ~~ ANY ('{autovacuum_analyze_scale_factor=%,autovacuum_analyze_threshold=%}') GROUP BY t.oid, st.schemaname, st.relname, t.reltuples, st.n_mod_since_analyze, st.last_autoanalyze) AS subq ORDER BY analyze_urgency DESC"

#define VISIBILITYMAPURGENCY_TITLE "Visibility Map urgency"
#define VISIBILITYMAPURGENCY_SQL "SELECT t.oid::regclass AS table_name, coalesce( vm.all_visible::float8 / nullif((rs.size / 8192)::float8, 0), 1.0) AS all_visible_factor FROM pg_class AS t CROSS JOIN LATERAL pg_visibility_map_summary(t.oid) AS vm CROSS JOIN LATERAL pg_relation_size(t.oid) AS rs(size) WHERE t.relkind = 'r' ORDER BY all_visible_factor"

#define XIDWRAPAROUNDURGENCY_TITLE "XID Wraparound Urgency"
#define XIDWRAPAROUNDURGENCY_SQL "SELECT datname, age(datfrozenxid) AS transaction_age, current_setting('autovacuum_freeze_max_age') FROM pg_database ORDER BY transaction_age DESC"

#define MXIDWRAPAROUNDURGENCY_TITLE "MXID Wraparound Urgency"
#define MXIDWRAPAROUNDURGENCY_SQL "SELECT datname, mxid_age(datminmxid) AS multixact_age, current_setting('autovacuum_multixact_freeze_max_age') FROM pg_database ORDER BY multixact_age DESC"
